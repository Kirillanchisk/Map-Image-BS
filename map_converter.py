...
def load_blocks():
    ...
    for filename in os.listdir(BLOCKS_DIR):
        if filename.endswith(".png"):
            ...
            cropped = resized.crop((0, 0, BLOCK_WIDTH, BLOCK_HEIGHT))

            avg_color = average_color(cropped)
...
def convert_image_to_map(image_path, output_path):
    ...
    for y in range(MAP_SIZE):
        for x in range(MAP_SIZE):
            ...
            if y == MAP_SIZE - 1:
                block_part = block.crop((0, 0, BLOCK_WIDTH, LAST_ROW_HEIGHT))
            else:
                block_part = block.crop((0, 0, BLOCK_WIDTH, BLOCK_HEIGHT))

            final_image.paste(block_part, (x * BLOCK_WIDTH, y * BLOCK_HEIGHT))
